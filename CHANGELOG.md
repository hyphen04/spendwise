...
- **Fixed a v2.14 → v3 upgrade that left the database un-openable (recovery loop).** Upgrading from v2.14 (schema v9) ran a migration that created `ai_threads` with the full v18 schema (incl. `pinned`/`archived`/`folder`) and then tried to `ADD COLUMN` those same columns — throwing "duplicate column name" and aborting before the schema version was bumped. The DB was left half-migrated at v9, so every launch re-crashed and the Database Recovery screen's restore (which copies an old replica back) just re-triggered the same crash. The v18 add-column step now checks `PRAGMA table_info` first and only adds what's missing. This both prevents the crash on fresh upgrades and **self-recovers** anyone already stuck: their next launch re-runs the migration, the no-op adds complete, and the version advances to v18 — no data loss, no recovery screen needed. No schema-version bump (target is still v18); the AI privacy boundary is untouched.
